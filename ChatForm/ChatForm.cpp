...
private:
	...
	void WriteFunction()
	{
		...
		std::wcout << L"Server: " << text << std::endl;
		...
		boost::asio::async_write(*socket.get(), boost::asio::buffer(text),
			[this](const boost::system::error_code& ec, std::size_t bytes)
			{
				if (ec.failed())
				{
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR nCmdLine, int nCmdShow)
{
	...
	std::shared_ptr<Chat> chat(std::make_shared<Chat>(hInstance, nCmdShow));
